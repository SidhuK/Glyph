...
<Steps>
  <Step title="Launch Glyph">
    Open Glyph for the first time. The launch screen lets you start immediately:

    - **Continue last space** - Reopen your most recent folder
    - **Open folder** - Use an existing local folder
    - **Create space** - Start a new folder for your notes

    Glyph keeps notes in local markdown files, so there is no import step to get started.
  </Step>
