...
pub fn query_tasks(root: &std::path::Path) -> Result<Vec<super::types::InboxTask>, String> {
    ...
    let missing: Vec<String> = {
        let mut statement = conn.prepare("SELECT n.path FROM notes n LEFT JOIN note_checklists c ON c.note_id = n.id AND c.etag = 'v3:' || n.etag WHERE c.note_id IS NULL").map_err(|error| error.to_string())?;
        let rows = statement
