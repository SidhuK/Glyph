...
pub fn is_flow_extension(ext: &str) -> bool {
...
pub fn is_flow_path(path: &Path) -> bool {
    path.extension()
        .and_then(|ext| ext.to_str())
        .map(|ext| is_flow_extension(ext))
        .unwrap_or(false)
}

pub fn is_indexable_document_path(path: &Path) -> bool {
    is_markdown_path(path) || is_flow_path(path)
}

pub fn is_wikilink_file_extension(ext: &str) -> bool {
    is_markdown_extension(ext) || is_pdf_extension(ext) || is_image_extension(ext) || is_flow_extension(ext)
}
