...
fn deny_hidden_rel_path(rel: &Path) -> Result<(), String> {
    for c in rel.components() {
        if c.as_os_str().to_string_lossy().starts_with('.') {
            return Err("hidden paths are not accessible".to_string());
        }
    }
    Ok(())
}
...
async fn search_vault(root: PathBuf, args: Value) -> Result<Value, String> {
    let query = args
        ...
        .to_string();
    if query.is_empty() {
        return Err("search_vault requires non-empty query".to_string());
    }
...
fn search_markdown_fallback(root: &Path, query: &str, limit: usize) -> Vec<Value> {
    ...
    while let Some(rel_dir) = stack.pop() {
        ...
        for entry in entries.flatten() {
            ...
            if !meta.is_file() || !is_markdown_path(&rel) || meta.len() > MAX_FALLBACK_FILE_BYTES {
                continue;
            }
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<Value, String> {
        ...
        out.sort_by(|a, b| {
            a.get("rel_path")
                .and_then(|v| v.as_str())
                .unwrap_or("")
                .cmp(b.get("rel_path").and_then(|v| v.as_str()).unwrap_or(""))
        });
...
async fn read_file(root: PathBuf, args: Value) -> Result<Value, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || -> Result<Value, String> {
        ...
        if total_bytes > MAX_READ_BYTES {
            return Err("file exceeds read cap".to_string());
        }
        let text = String::from_utf8(std::fs::read(&abs).map_err(|e| e.to_string())?)
