...
use super::types::{AiChunkEvent, AiMessage, AiProfile, AiProviderKind, AiToolEvent, AgentToolCall};

const MAX_AGENT_STEPS: usize = 6;
const MAX_TOOL_RESULT_CHARS: usize = 120_000;
const FAKE_CHUNK_CHARS: usize = 140;
...
pub async fn run_agent_loop(
    ...
    for _ in 0..MAX_AGENT_STEPS {
        ...
        if cancelled { return Ok((String::new(), true)); }
        match parse_output(&raw) {
            AgentOutput::ToolCall(call) => {
                emit_tool(app, job_id, &call.name, "call", call.call_id.clone(), Some(call.args.clone()), None);
                ...
                match result {
                    Ok(payload) => {
                        ...
                        if total_tool_chars > MAX_TOOL_RESULT_CHARS {
                            ...
                            emit_fake_chunks(app, job_id, text);
                            return Ok((text.to_string(), false));
                        }
                        emit_tool(app, job_id, &call.name, "result", call.call_id.clone(), Some(payload.clone()), None);
                    ...
                    Err(err) => {
                        ...
                        emit_tool(app, job_id, &call.name, "error", call.call_id.clone(), None, Some(err.clone()));
                        convo.push(tool_result_message(&call, None, Some(err)));
                    }
                }
            }
            AgentOutput::Final(text) | AgentOutput::Plain(text) => { emit_fake_chunks(app, job_id, &text); return Ok((text, false)); }
        }
    }
...
fn parse_output(raw: &str) -> AgentOutput {
    ...
    if ty == "final" {
        let text = v.get("text").or_else(|| v.get("content")).or_else(|| v.get("message")).and_then(|v| v.as_str()).unwrap_or("").to_string();
...
fn emit_fake_chunks(app: &AppHandle, job_id: &str, text: &str) {
