...
{
    let (mut cancelled, mut full, mut buf, mut stream) =
        (false, String::new(), String::new(), resp.bytes_stream());
    loop {
        tokio::select! {
...
pub async fn stream_openai_like(
    ...
    stream_sse(cancel, app, job_id, emit_chunks, resp, |data, _| {
        ...
        if delta.is_empty() {
            ...
            SseAction::Append(delta.to_string())
        }
    })
    .await
}
