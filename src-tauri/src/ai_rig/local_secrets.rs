...
pub fn secret_get(space_root: &Path, profile_id: &str) -> Result<Option<String>, String> {
    let path = secrets_path(space_root)?;
    let map = read_map(&path);
    Ok(map.get(profile_id).map(ToString::to_string))
}

pub fn secret_set(space_root: &Path, profile_id: &str, secret: &str) -> Result<(), String> {
...
pub fn secret_clear(space_root: &Path, profile_id: &str) -> Result<(), String> {
    let path = secrets_path(space_root)?;
    let mut map = read_map(&path);
