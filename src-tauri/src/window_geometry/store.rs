...
struct WindowGeometryStoreVersion {
    version: u32,
}
...
pub(super) fn load_record(path: &Path) -> Result<Option<WindowGeometryRecord>, String> {
    match std::fs::read(path) {
        Ok(bytes) => {
            let stored_version: WindowGeometryStoreVersion =
                serde_json::from_slice(&bytes).map_err(|error| error.to_string())?;
            if stored_version.version < WINDOW_GEOMETRY_STORE_VERSION {
                return Ok(None);
            }
            if stored_version.version > WINDOW_GEOMETRY_STORE_VERSION {
                return Err(format!(
                    "unsupported window geometry store version {} (max supported {})",
                    stored_version.version, WINDOW_GEOMETRY_STORE_VERSION
                ));
            }
...
pub(super) fn save_record(path: &Path, record: &WindowGeometryRecord) -> Result<(), String> {
