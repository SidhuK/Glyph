...
import { extractMermaidErrorMessage, isMermaidCodeBlockLanguage } from "./mermaid";
...
describe("mermaid helpers", () => {
	...
	it("extracts user-facing Mermaid render errors", () => {
		expect(extractMermaidErrorMessage(new Error(" Bad syntax "))).toBe("Bad syntax");
		expect(extractMermaidErrorMessage(" Parse failed ")).toBe("Parse failed");
		expect(extractMermaidErrorMessage(null)).toBe("Unable to render Mermaid diagram.");
	});
});
