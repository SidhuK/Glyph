...
describe("ColoredText markdown integration", () => {
	it("round-trips colored spans through markdown parse and serialize", () => {
		...
		const input =
			'Hello <span data-glyph-color="blue" style="color: var(--glyph-inline-color-blue, #1a6ba4)">world</span>';
	...
	it("preserves nested formatting inside colored text", () => {
		const manager = createMarkdownManager();
		const input =
			'Before <span data-glyph-color="red" style="color: var(--glyph-inline-color-red, #c73838)">**alert**</span> after';
	...
	it("applies color as a stored mark for collapsed selections and can clear it", () => {
		...
		expect(postprocessMarkdownFromEditor(editor.getMarkdown())).toBe(
			'<span data-glyph-color="green" style="color: var(--glyph-inline-color-green, #0f7b0f)">done</span>',
		);
