...
export const RawMarkdownEditor = forwardRef<RawMarkdownEditorHandle, RawMarkdownEditorProps>(
	function RawMarkdownEditor({ markdown, relPath, onChange }, forwardedRef) {
		...
		useLayoutEffect(() => {
			...
			view.dispatch({
				changes:
					currentMarkdown === markdown ? undefined : diffMarkdownChange(currentMarkdown, markdown),
				selection,
				effects: didChangeDocument ? EditorView.scrollIntoView(0, { y: "start" }) : [],
