...
describe("looksLikeMarkdownPaste", () => {
	...
	it("detects inline markdown constructs", () => {
		...
		expect(looksLikeMarkdownPaste("Read [Glyph](https://example.com)")).toBe(true);
		expect(looksLikeMarkdownPaste("![Diagram](https://example.com/diagram.png)")).toBe(true);
		expect(looksLikeMarkdownPaste("Jump to [[Daily Note]]")).toBe(true);
	});

	it("ignores ordinary prose", () => {
		expect(
			looksLikeMarkdownPaste("This is regular pasted text with no formatting syntax at all."),
		).toBe(false);
		expect(looksLikeMarkdownPaste("Budget is 1.5x higher than last quarter.")).toBe(false);
